...
import { 
  Sparkles, Moon, Sun, Heart, Eye, Wind,
...
const GuidedMeditation = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying && selectedScript && time < selectedScript.duration) {
      interval = setInterval(() => setTime(t => t + 1), 1000)
    } else if (time >= (selectedScript?.duration || 0)) {
      setIsPlaying(false)
    }
  ...
  useEffect(() => {
    ...
    if (phase >= 0) setCurrentPhase(phase)
  }, [time, selectedScript])
      ...
      <div className="flex justify-center gap-4">
        <Button
          ...
          className={cn(
            ...
            isPlaying ? "bg-amber-500 hover:bg-amber-600" : `bg-${selectedScript.color}-500`
          )}
        >
